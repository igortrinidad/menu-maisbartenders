...
var openFB = (function () {
    ...
    function login(callback, options) {
        ...
        loginWindow = window.open(loginURL + '?client_id=' + fbAppId + '&redirect_uri=' + redirectURL +
            '&response_type=token&scope=' + scope, '_blank', 'location=no,clearsessioncache=yes"');
    ...
    function logout(callback) {
        ...
        if (token) {
            logoutWindow = window.open(logoutURL + '?access_token=' + token + '&next=' + logoutRedirectURL, '_blank', 'location=no,clearsessioncache=yes"');
            if (runningInCordova) {
                setTimeout(function() {
                    logoutWindow.close();
                }, 700);
